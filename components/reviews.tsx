    ...
    <div className="bg-white border border-slate-100 rounded-3xl p-8 shadow-sm space-y-8">
      ...
      <div className="space-y-6">
        {initialReviews.map((review) => (
          <div key={review.id} className="flex gap-4 border-b border-slate-50 pb-6 last:border-0">
            <Link href={`/profile/${review.user?.profileToken}`} className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center font-bold text-xs hover:ring-2 hover:ring-orange-500 transition-all">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex gap-0.5 mt-0.5">
                      {[1,2,3,4,5].map(s => (
                        <LucideStar key={s} className={`w-3 h-3 ${s <= review.rating ? "fill-orange-400 text-orange-400" : "text-slate-200"}`} />
                      ))}
                    </div>
